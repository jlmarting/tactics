import { ColliderToken } from "../lib/tokens/collidertoken.js";
import { Point2D } from "../lib/tokens/components/point2d.js";
...
Population.populateScene = function (scene, build) {
    ...
    if (build == 'general') {
        ...
        for (var i = 0; i < 1000; i++) {
            for (var j = 10; j < 10; j++) {
                var theWire = new WireToken('wire1', new Point2D(i, j));
                theWire.points.push(new Point2D(i + 32, j));
                theWire.points.push(new Point2D(i + 32, j + 20));
                theWire.points.push(new Point2D(i, j + 20));
                theWire.points.push(new Point2D(i, j));
                scene.arrTokens.push(theWire);
            }
        }
        wallPos = { x: -500, y: -200 };
        for (var i = 0; i < 10; i++) {
        ...
        var theWire = new WireToken('wire1_', new Point2D(wallPos.x, wallPos.y));
        theWire.load(new Point2D(16, 10));
        theWire.load(new Point2D(-32016, 10));
        theWire.load(new Point2D(-32016, 10));
        theWire.load(new Point2D(16, -216));
        scene.arrTokens.push(theWire);
        scene.arrTokens.push(theBlock2);
        scene.arrTokens.push(theBlock1);
        scene.arrTokens.push(theToken);
        scene.arrTokens.push(theGrass1);
        scene.arrTokens.push(theGrass2);
        scene.arrTokens.push(theGrass3);
        scene.arrTokens.push(theGrass4);
        scene.arrTokens.push(theBlock3);
        scene.arrTokens.push(theBlock4);
        scene.setToken("one");
    }
    if (build == 'linerectangle') {
        var wire1 = new WireToken('wire1', new Point2D(0, 0));
        wire1.load(new Point2D(-551, 351));
        wire1.load(new Point2D(600, 250));
        ...
        scene.arrTokens.push(rectangle);
        var msg = "";
