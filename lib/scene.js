const Scene = function(canvasId){    //Lo que veremos en el canvas   
    ...
    this.config = {viewLines:true, scale:1, viewColliders: false, viewIds: true, autoFPS: false, viewPortWidth: 1400, viewPortHeight: 800};
    this.config.grid = {width: 1000, granularity: 25};
    ...
    this.drawLines = function(){        
        ...
        for(var x=-grid.width;x<=grid.width;x+=grid.granularity){
            for(var y=-grid.width;y<=grid.width;y+=grid.granularity){
                var xr = x+self.x;
                var yr = y+self.y;
                if((x%(grid.granularity*4)==0)&&(y%(grid.granularity*4)==0)){
                    self.ctx.fillRect(xr-5,yr-5,5,5);
                    if((x%(grid.granularity*5)==0)&&(y%(grid.granularity*5)==0)){
                        self.ctx.fillText(`(${x},${y})`,xr,yr);
                    }
                }
                else {
                    self.ctx.fillRect(xr,yr,1,1);
                }           
                self.ctx.font = '12px';                
            }
            
        }
        self.ctx.fillText( 'CEROOOOOOOO' ,0,0);                
    }
    ...
    Point.prototype.draw = function(lColor,fColor){
        ...
        if(this.config.position == 'relative'){
            self.ctx.fillRect(this.x+self.x, this.y+self.y, 2,2);
