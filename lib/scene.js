const Scene = function(canvasId){    //Lo que veremos en el canvas   
    ...
    this.config = {viewLines:false, scale:1.5, viewColliders: false, viewIds: true};
    ...
    this.resize = function(){
        self.canvas.width =  (window.innerWidth)*0.96;
        self.canvas.height = (window.innerHeight)*0.96;
        ...
        self.mapkey.forEach(function(cmd){   
            ...
            if(cmd=="fire"){
                if(self.arrTokens[self.tokenIndex] instanceof Shooter){
                    var bullet = self.arrTokens[self.tokenIndex].shot(function(collisions){//Callback, se ejecutará cuando la bala impacte
                                                                if(typeof collisions == 'undefined'){
                                                                    console.log('callback de disparo sin colisiones?');
                                                                    return false;
                                                                }
                                                                ...
                                                                thPromise(collisions).then(function(tokenHitted){
                                                                    ...
                                                                    if (tokenHitted.health<=0){
                                                                        var tokenIndex = self.arrTokens.findIndex(function(element){
                                                                            return element.id == tokenHitted.id;
                                                                        });                                                                   
                                                                        self.arrTokens.splice(tokenIndex,1);
                                                                        self.tokenIndex = self.arrTokens.findIndex(function(element){
                                                                            return element.id == self.tokenId;
                                                                        });
                                                                    }
                                                                });
