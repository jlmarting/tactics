const Scene = function(canvasId){    //Lo que veremos en el canvas   
    ...
    this.pause = false;
    this.config = {viewLines:false, scale:1.5, viewColliders: false, viewIds: true, autoFPS: false};
    ...
    //Definición de callbacks por impacto ()
    const bulletEffect = function(collisions,bullet){//Callback, se ejecutará cuando la bala impacte                                                                
        ...
        self.mapkey.forEach(function(cmd){   
            ...
            if(cmd=="fire"){
                if(self.arrTokens[self.tokenIndex] instanceof Shooter){
                    ...
                    var bullet = self.arrTokens[self.tokenIndex].shot(bulletEffect);
                ...
                //Movemos displ*delta, dado que displ representa el desplazamiento por unidad de tiempo
                //por tanto moveremos más si el lapso de tiempo es mayor
                self.arrTokens[self.tokenIndex].move(cmd, self.arrTokens[self.tokenIndex].displ*(delta),self.arrTokens);                
    ...
    this.drawScene = function(timeStamp){         
        ...
        //Info
        document.getElementById('info').value = `Tokens: [${self.arrTokens.length}]FPS: ${self.fps} min.elapsed: ${Math.round(1000/self.fps)} realFPS: ${realFPS} av.elapsed = ${Math.round(averageInterval)}`;
