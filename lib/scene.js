var Scene = function(canvasId){    //Lo que veremos en el canvas   
    ...
    this.config = {viewLines:false, scale:0.80, viewColliders: false, viewIds: true};
    ...
    this.drawPath = function(){
            if(self.getSelectedToken() instanceof Collider){
                self.ctx.strokeStyle = '#ff00f0';
                self.ctx.linewidth = 1;
                self.ctx.beginPath();           
                self.arrTokens[self.tokenIndex].collider.back.forEach(function(pos){                    
                var rpos = {x:pos.x+self.x,y:pos.y+self.y};                      
                self.ctx.lineTo(rpos.x,rpos.y);                             
                self.ctx.arc(rpos.x,rpos.y, 5, 0, Math.PI*2,false);  
                self.ctx.stroke();  
            });
            }
    ...
    this.drawLines = function(){
        ...
        var oTokenCenter = self.arrTokens[self.tokenIndex].getRelPos();
    ...
    // Implementación de DRAW para todos los elementos representables
    ...
    Point.prototype.draw = function(lColor,fColor){
        ...
        self.ctx.fillRect(this.x+self.x, this.y+self.y, 5,5);        
        self.ctx.stroke();    
    }
    ...
    this.drawScene = function(){    //Ciclo de dibujo (activamos movimientos automáticos, pintamos en canvas)        
        ...
        self.mapkey.forEach(function(cmd){   
            ...
            if(cmd=="fire"){
                if(self.arrTokens[self.tokenIndex] instanceof Shooter){
                    var bullet = self.arrTokens[self.tokenIndex].shot(function(){
                        console.log(`Shot ${self.id} ended!`);
                        var i = self.arrTokens.indexOf(this);
                        self.arrTokens.splice(i,1);   
                    });
                    if(bullet instanceof Point){
                        console.log(`scene.fire instaceof Point-> ${JSON.stringify(bullet)}`);
                        self.arrTokens.push(bullet);                
                    }else{
                ...
                self.arrTokens[self.tokenIndex].move(cmd, self.arrTokens[self.tokenIndex].displ,self.arrTokens);                
        ...
        if(self.zoomout){
            ...
            self.config.scale = self.config.scale * 1.2;
            self.ctx.scale(self.config.scale,self.config.scale);
            self.zoomout = false;
        }
        ...
        if (difT>=1000){                                    
            ...
            var pos = self.arrTokens[self.tokenIndex].getCenter();

            var msg = 'fps:' + fps.toString().substr(0,6) + ' - ' + difT + ' ms, ' 
            + self.arrTokens.length  +'tokens.';
            
            if(self.arrTokens[self.tokenIndex] instanceof ColliderToken){
                msg = msg + ' Angle: ' + self.arrTokens[self.tokenIndex].rad.toFixed(3) + ' (col: ' + self.arrTokens[self.tokenIndex].collider.rad.toFixed(3) + ')'
            + ' Token pos: (' + Math.round(pos.x) + ',' + Math.round(pos.y) +')' 
            + ' Main col.pos: [' + Math.round(self.arrTokens[self.tokenIndex].collider.x) + ',' + Math.round(self.arrTokens[self.tokenIndex].collider.y) + ']';

            }
            
            msg = msg + ' ZOOM: ' + self.config.scale;
