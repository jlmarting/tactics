...
var Collider = function(id,x,y,rad,cradius){
    ...
    //Redimensiona el colisionador. Especialmente útil cuando
    // cargamos una imagen y ya tenemos dimensiones
...
Collider.prototype.move = function(cmd,displ,tokens){
    ...
    if(arrColId.length>0){ //TODO: ignoramos colisiones de balas..
        this.x = this.backX;
        this.y = this.backY;        
...
ColliderToken.prototype.move = function(cmd,displ,tokens){
    ...
    if (d<=0){
        ...
        this.move(cmd,((displ)*(-1.2)),tokens);   //TODO: mejorar este comportamiento en caso de colisión       
    }else{                        
...
var Projectile = function(x,y,rad,displ,callback){
    Point.call(this,x,y);    
    this.rad = rad;        
    this.displ = displ;
    this.range = 800; //rango corto para pruebas
    this.callback = callback; //función a ejecutar en un determinado momento (ej: colisión)
}
...
Projectile.prototype.shot = function(){
                                var d = this.move();
                                if((this.range>0) && (this.displ==0)){this.range=0;this.callback();d=-1;}
                                return d;
                            }
                        


var BulletProjectile = function(id,x,y,rad,displ,callback){
    Projectile.call(this,x,y,rad,displ,callback);    
    this.collider = new Collider(id,x,y,rad,5);    
}

BulletProjectile.prototype = Object.create(Projectile.prototype);

BulletProjectile.prototype.shot = function(tokens,callback){   
    var d = Collider.prototype.move.call(this.collider,"up",this.displ, tokens);           
    if((this.range<=0)||(d==-1)){this.displ=0;}
        Projectile.prototype.shot.call(this,callback);
    return this.range;
}

//Redefinimos placeAt dado que este colisionador no se calcula en función de una imagen
BulletProjectile.prototype.placeAt = function(x,y){
Point.prototype.placeAt.call(this,x,y); 
    this.collider.reloadCollider(x,y,15,15);
}
...
Shooter.prototype.shot = function(){
    var shotLapse = 150;
    var shotDispl = 15;
    var callback = function(){console.log('booom!');}
    if(this.reloading==true) return false;
    ...
    var bullet = new BulletProjectile(this.id, (xy.x), (xy.y), this.rad, shotDispl,callback);
    this.scene.arrTokens.push(bullet);
    bullet.shot(this.scene.arrTokens);
    this.reloading = true;
    var shooter = this;
