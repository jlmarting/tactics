...
export class Scene {
    ...
    drawScene(timeStamp) {
        ...
        if (infoTokens)
            infoTokens.innerText = `TOKENS: [${this.arrTokens.length} total / ${this.arr.length} render]`;
        const infoFPS = document.getElementById('info-fps');
        if (infoFPS)
            infoFPS.innerText = `FPS: [${this.fps} cfg / ${realFPS} real]`;
        const infoCycle = document.getElementById('info-cycle');
        if (infoCycle)
            infoCycle.innerText = `Ciclo: [${Math.round(1000 / this.fps)}ms cfg / ${Math.round(averageInterval)}ms real]`;
        const infoEngine = document.getElementById('info-engine');
        if (infoEngine)
            infoEngine.innerText = `Log: ${this.engineInfo}`;
        const sceneMsg = document.getElementById('scene-message');
        if (sceneMsg)
            sceneMsg.innerText = this.message;
    ...
    setupUIControls() {
        ...
        const collision = getElem('collision');
        if (collision) {
            collision.onchange = () => {
        ...
        const tokenSelector = getElem('tokens');
        if (tokenSelector) {
