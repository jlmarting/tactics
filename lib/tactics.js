...
function Token(id,x,y,rad,src,scene){
    ...
    this.img.x = this.x;
    this.img.y = this.y;
    //this.collider = new CCollider(this.x + (this.img.width/2),this.y + (this.img.height/2),50, scene.canvas);
    ...
    this.move = (function(keyCode){
                    ...
                    scene.arrTokens.forEach((function(item){
                        if(this.id!==item.id){ 
                            if (this.collider.isCollisioning(item.collider)){
                                console.log('Hay colisión con ' + item.id + ' collider: ' + item.collider.x +' ' + item.collider.y + ' ' + item.collider.radius);                                
                                colId = item.id;
                                this.collider.draw(myCanvas);
                                item.collider.draw(myCanvas);
                            }
                            else {
                                console.log('No colisiona con ' + item.id + ' collider: ' + item.collider.x +' ' + item.collider.y + ' ' + item.collider.radius);
                            }
                        }
                    }).bind(this));
                    ...
                    if(colId == this.id){ // mismo id, sin colisión
                        ...
                        this.img.x = this.x;
                        this.img.y = this.y;
                        console.log(this.id + ' sin colisiones');    
                    }else{ //deshacemos
                        this.x -= dx;
                        this.y -= dy; 
                        this.collider.x -= dx;
...
var CCollider = function(cx,cy,cradius,canvas){
    ...
    this.isCollisioning = function(otherCollider){
        var dx = this.x - otherCollider.x; console.log(this.x);
        var dy = this.y - otherCollider.y; console.log(this.x);
        var distance = Math.sqrt((dx*dx)+(dy*dy));
        console.log(distance +' ' + (this.radius + otherCollider.radius));
    ...
    this.draw = function(canvas){
        ...
        ctx.stroke();
        console.log('Dibujando collider');
    }
