...
window.onload = function(){
    ...
    var wallPos = {x:-100,y:-200}
    for(var i=0;i<20;i++){
        for(var j=0;j<3;j++){
            var brick = new ColliderToken('brick_'+i+'_'+j,wallPos.x+(32*i),wallPos.y+(20*j) ,0,'img/brick001_32x20.png',32,20);
            brick.health = 150;
    ...
    for(var i=0;i<3;i++){
        for(var j=0;j<20;j++){
            var brick = new ColliderToken('brick_'+i+'_'+j,wallPos.x+(20*i),wallPos.y+(32*j) ,1.5708,'img/brick001_32x20.png',32,20);
            brick.health = 150;
            brick.config.viewName = false;
            theScene.arrTokens.push( brick);            
        }        
