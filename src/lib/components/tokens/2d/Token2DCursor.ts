...
import { IMovable2D } from "../../../interfaces/IMovable2D.js";
import { IKeyBindeable } from "../../../interfaces/IKeyBindeable.js";
import { WithKeyBinding } from "../../../tools/decorators/WithKeyBinding.js";
...
// @WithKeyBinding
export class Token2DCursor extends Token2D implements IMovable2D{
    //Orientación, radianes
    rad: number;
    
    grad: number;
    //Incremento de orientación por movimiento
    incrGrad: number;
    //Desplazamiento por movimiento
    displ: number;
    //Registro de posiciones previas
    path: Coord2D[];
    //Asociación de teclas
    keyBinder: IKeyBindeable<Movement2D>;

    key: KeyCmd;
    
    constructor(position: Coord2D, rad:number) {
        super(position);
        this.incrGrad = 10;
        this.displ = 1;
        this.path = [];
        ...
        this.turn(rad);
        this.keyBinder = new KeyBinder<Movement2D>();
